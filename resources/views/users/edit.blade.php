                            ...
                            <div class="col-md-12">
                                ...
                                <div class="form-check form-switch mt-1">
                                    <input class="form-check-input" type="checkbox" role="switch" id="is_active"
                                        v-model="form.is_active" :disabled="isSelf">
                                    <label class="form-check-label" for="is_active">
                                        @{{ form.is_active ? 'Active' : 'Inactive' }}
                                    </label>
                                </div>
                                <small v-if="isSelf" class="text-muted">You cannot deactivate your own account.</small>
                            </div>
...
<script>
    $(function () {
        ...
        const assigned      = @json($assignedRoleIds);
        const assignedLocs  = @json($assignedLocationIds);
        const isSelf        = {{ auth()->id() === $user->id ? 'true' : 'false' }};
        ...
        new Vue({
            ...
            data: {
                ...
                form: {
                    ...
                    location_ids: assignedLocs.map(Number),
                    password: '',
                    password_confirmation: '',
                    is_active: {{ $user->is_active ? 'true' : 'false' }},
                    role_ids: assigned.map(Number),
                },
                errors: {},
                submitting: false,
                wasValidated: false,
            ...
            methods: {
                validateLocal() {
                    ...
                    if (this.form.password && this.form.password.length < 8) {
                        this.$set(this.errors, 'password', 'Minimum 8 characters.');
                    }
