...
<div style="height:140px;display:flex;flex-direction:column;align-items:center;justify-content:center;background:linear-gradient(135deg,var(--dark-900),var(--dark-750));border-bottom:1px solid rgba(214,48,140,.1);text-align:center">
...
<div class="sg-orders-wrap" style="padding:48px 60px;background:var(--dark-900);min-height:60vh">
  <div style="max-width:1100px;margin:0 auto">
    ...
    @if($orders->isEmpty())
      <div style="background:var(--dark-800);border:1px solid rgba(214,48,140,.1);border-radius:6px;padding:80px;text-align:center">
    ...
    @else
      <div style="background:var(--dark-800);border:1px solid rgba(214,48,140,.1);border-radius:6px;overflow:hidden">
        ...
        <table style="width:100%;border-collapse:collapse;min-width:820px">
          <thead>
            <tr style="background:rgba(214,48,140,.04);font-size:11px;letter-spacing:1.5px;text-transform:uppercase;color:var(--white-faint)">
              ...
              <tr style="border-top:1px solid rgba(214,48,140,.06);transition:background .2s" onmouseover="this.style.background='rgba(214,48,140,.04)'" onmouseout="this.style.background='transparent'">
                ...
                <td style="padding:16px 22px;text-align:center">
                  <a href="{{ route('website.account.order-detail', $order->id) }}"
                     style="font-size:11px;letter-spacing:1.5px;text-transform:uppercase;color:var(--teal-300);text-decoration:none;padding:6px 14px;border:1px solid rgba(214,48,140,.3);border-radius:2px;transition:all .2s"
                     onmouseover="this.style.background='rgba(214,48,140,.07)'" onmouseout="this.style.background='transparent'">
                    View
                  </a>
                </td>
              </tr>
      ...
      @if($orders->hasPages())
        <div style="margin-top:24px;display:flex;justify-content:center;gap:8px">
          ...
          @else
            <a href="{{ $orders->previousPageUrl() }}" style="padding:8px 16px;font-size:12px;color:var(--teal-300);border:1px solid rgba(214,48,140,.25);border-radius:2px;text-decoration:none;transition:all .2s">← Prev</a>
          @endif
          @foreach($orders->getUrlRange(1, $orders->lastPage()) as $page => $url)
            ...
            @else
              <a href="{{ $url }}" style="padding:8px 14px;font-size:12px;color:var(--teal-300);border:1px solid rgba(214,48,140,.25);border-radius:2px;text-decoration:none;transition:all .2s">{{ $page }}</a>
            @endif
          @endforeach
          @if($orders->hasMorePages())
            <a href="{{ $orders->nextPageUrl() }}" style="padding:8px 16px;font-size:12px;color:var(--teal-300);border:1px solid rgba(214,48,140,.25);border-radius:2px;text-decoration:none;transition:all .2s">Next →</a>
          @else
