...
{{-- HERO BAND --}}
<div class="sg-coll-hero" style="display:flex;flex-direction:column;align-items:center;justify-content:center;background:linear-gradient(135deg,var(--dark-900),var(--dark-750));border-bottom:1px solid rgba(0,191,176,.1);text-align:center;position:relative;overflow:hidden">
...
{{-- FILTER BAR --}}
<div class="sg-filter-bar sg-sec-px" style="display:flex;align-items:center;gap:12px;padding:16px 0;background:var(--dark-800);border-bottom:1px solid rgba(0,191,176,.07);flex-wrap:wrap">
  {{-- Category chips --}}
  <a class="sg-chip" href="{{ route('website.collections', array_merge(request()->except('category','page'), ['sort' => $sort])) }}"
     style="border-radius:20px;font-size:12px;font-weight:500;letter-spacing:1px;text-transform:uppercase;cursor:pointer;border:1px solid rgba(0,191,176,.25);text-decoration:none;transition:all .3s;{{ !$categorySlug ? 'background:rgba(0,191,176,.1);border-color:var(--teal-400);color:var(--teal-300)' : 'color:rgba(240,250,248,.35)' }}">All</a>
  ...
  <a class="sg-chip" href="{{ route('website.collections', array_merge(request()->except('category','page'), ['category' => $catCode, 'sort' => $sort])) }}"
     style="border-radius:20px;font-size:12px;font-weight:500;letter-spacing:1px;text-transform:uppercase;cursor:pointer;border:1px solid rgba(0,191,176,.25);text-decoration:none;transition:all .3s;{{ $categorySlug === $catCode ? 'background:rgba(0,191,176,.1);border-color:var(--teal-400);color:var(--teal-300)' : 'color:rgba(240,250,248,.35)' }}">
    {{ $cat->name }}
  ...
  <form method="GET" action="{{ route('website.collections') }}" class="sg-search-form" style="display:flex;gap:0;margin-left:auto">
    ...
    <input name="q" value="{{ $search }}" placeholder="Search gems…" class="sg-search-input"
      style="background:var(--dark-750);border:1px solid rgba(0,191,176,.2);border-right:none;color:#f0faf8;font-family:'Jost',sans-serif;font-size:13px;outline:none;border-radius:2px 0 0 2px">
  ...
  <select onchange="window.location=this.value" class="sg-sort-select"
    style="background:var(--dark-750);border:1px solid rgba(0,191,176,.2);color:#f0faf8;font-family:'Jost',sans-serif;font-size:13px;border-radius:2px;outline:none;cursor:pointer">
