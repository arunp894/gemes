...
class UpdateSupplierRequest extends FormRequest
{
    ...
    {
        $nullable = [
            'company_name', 'invoice_prefix', 'email', 'alternate_phone',
    ...
    {
        // Route-model binding gives us the Supplier instance; fall back to raw id.
        $supplierId = $this->route('supplier')?->id ?? $this->route('supplier');

        return [
            // Code is treated as immutable on the form (read-only), but the
            // rule still tolerates a posted value as long as it doesn't
            // collide with another supplier.
            'supplier_code' => [
                'sometimes',
                'required',
                'string',
