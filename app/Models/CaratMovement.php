...
/**
 * CaratMovement — single row in the append-only CT (carat) ledger.
 *
 * Mirrors StockMovement exactly, but tracks carat weight instead of
 * piece count. Exists because a purchase_products row's qty and its
 * carat_weight are independent: a qty=3 row can be 20ct + 7ct + 3ct
 * rather than 10ct each, so CT needs its own balance, not a derived
 * qty × carat_weight figure.
 *
 ...
 * exactly like StockMovement.
 *
 * The math everywhere is:
 *   balance = SUM(carat WHERE direction='in') − SUM(carat WHERE direction='out')
 */
class CaratMovement extends Model
{
    use HasFactory;
    use SoftDeletes;
    ...
    /* ─── Reasons — mirrors StockMovement::REASON_* ──────────── */
    ...
    protected static function booted(): void
    {
        ...
        static::updating(function (self $m) {
            $protected = [
                ...
                'direction', 'carat', 'reason',
                'source_type', 'source_id', 'source_line_id',
            ];
            foreach ($protected as $col) {
                if ($m->isDirty($col)) {
                    throw new \LogicException(
                        "CaratMovement is append-only. Column '{$col}' cannot be modified. "
    ...
    public function signedCarat(): float
    {
        return $this->isIn() ? (float) $this->carat : -((float) $this->carat);
    }
